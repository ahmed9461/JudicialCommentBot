...
_BOG_CASE_PATTERN = re.compile(
    rf"رقم\s*القضية\s*في\s*المحكمة\s*(?:الإدارية|الادارية)\s*[:：\-]?\s*({_ID}?)"
    rf"(?=\s*(?:لعام|عام|رقم\s*القضية\s*في\s*محكمة\s*(?:الاستئناف|االستئناف)|تاريخ\s*الجلسة|الموضوعات|مستند\s*الحكم|$))",
    re.I | re.S,
)
_CRSD_CASE_PATTERN = re.compile(
    rf"رقم\s*القضية\s*(?:لدى|أمام|امام)\s*لجنة\s*الفصل(?:\s*في\s*منازعات\s*الأوراق\s*المالية)?"
    rf"\s*[:：\-]?\s*[\(\)（）]*({_SIMPLE_ID})",
    re.I,
)
...
_IDC_MARKERS = (
    ...
    "لجنة الفصل في المنازعات والمخالفات التامينية",
    "اللجنة الفصل في المنازعات والمخالفات التامينية",
