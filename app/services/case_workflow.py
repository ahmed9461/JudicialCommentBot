...
class CaseWorkflowService:
    ...
    async def prepare(self, user_id: int, subject_slug: str) -> PreparedBatch:
        async with self._lock(user_id):
            ...
            exclusions = await self.database.used_cases_for_subject(subject_slug)
            dynamic_exclusions = list(exclusions)
            verified: list[PreparedCase] = []
            seen_identity: set[tuple[str, str] | tuple[str, str, str]] = set()
            ...
            if not verified:
                raise NoSuitableCasesError(
                    "No candidate survived official PDF verification and deduplication"
                )
    ...
    async def _verify_candidate(self, candidate: CaseCandidate) -> PreparedCase | None:
        ...
        if not pdf_url and self.source_registry.can_be_original_pdf_source(
            candidate.source_url_str
        ):
        ...
        try:
            ...
            if artifact.page_count > self.settings.compilation_page_threshold:
                if not candidate.has_page_range:
                    raise PdfValidationError(
                        "Large official compilation requires a verified case page range"
                    )
            ...
            if await self.database.is_case_used(
                ...
                logger.info("Candidate rejected as duplicate: %s", candidate.case_number)
                artifact.path.unlink(missing_ok=True)
                return None

            text = extract_pdf_text(
                artifact.path, max_chars=self.settings.commentary_input_max_chars
            )
            ...
            classification = self.source_registry.classify(artifact.source_url)
            score = self.scoring.score(
                candidate, pdf_is_official=classification.is_official
            )
            return PreparedCase(
        ...
        except (PdfAcquisitionError, PdfValidationError, OSError, ValueError) as exc:
    ...
    def _extract_compilation_case(
        self, artifact: PdfArtifact, candidate: CaseCandidate
    ) -> PdfArtifact:
        if not candidate.has_page_range:
            raise PdfValidationError("Compilation case page range is missing")
        assert candidate.pdf_page_start is not None
        assert candidate.pdf_page_end is not None
    ...
    @staticmethod
    def _candidate_identity(candidate: CaseCandidate):
    ...
    async def record_sent(self, user_id: int) -> None:
        ...
        if not session.recorded:
            await self.database.record_case(
                ...
                source_url=selected.candidate.source_url_str,
