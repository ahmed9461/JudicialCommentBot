...
class FakePdfService:
    ...
    async def acquire(self, url: str, *, suggested_name: str = "case", progress=None):
        ...
        return PdfArtifact(path=path, source_url=url, sha256="c" * 64, size_bytes=len(payload), page_count=2)
...
class CatalogPdfService:
    ...
    async def acquire(self, url: str, *, suggested_name: str = "case", progress=None):
        ...
        with path.open("wb") as handle:
            writer.write(handle)
        payload = path.read_bytes()
        return PdfArtifact(path=path, source_url=url, sha256=self.sha256, size_bytes=len(payload), page_count=4)
...
def catalog_candidate(number: str = "777") -> CaseCandidate:
    return CaseCandidate(
        title=f"قضية {number}", case_number=number, court_name="المحكمة الجزائية",
        source_name="وزارة العدل", source_url="https://www.moj.gov.sa/collection.pdf",
        pdf_url="https://www.moj.gov.sa/collection.pdf", pdf_page_start=2, pdf_page_end=3,
        catalog_key="catalog-fixture", catalog_pdf_sha256="d" * 64, catalog_range_verified=True,
        legal_issue="مسألة قانونية واضحة", suitability_reason="صلة مباشرة بموضوع المقرر",
        estimated_score=93, subject_relevance=38, legal_issue_clarity=19,
        reasoning_quality=14, academic_commentary_value=14,
    )
...
@pytest.mark.asyncio
async def test_verified_catalog_range_uses_fingerprint_and_does_not_refine_hint(tmp_path: Path, monkeypatch) -> None:
    db = Database(f"sqlite+aiosqlite:///{tmp_path / 'catalog-workflow.db'}")
    await db.initialize()
    settings = Settings(telegram_bot_token="123456:fixture", owner_telegram_id=1, temp_dir=str(tmp_path), candidate_display_count=1, search_candidate_limit=1, commentary_min_text_chars=10)
...
async def test_stale_catalog_fingerprint_is_rejected_before_page_use(tmp_path: Path) -> None:
    ...
    await db.initialize()
    service = CaseWorkflowService(database=db, subject_loader=SubjectLoader(), research_provider=FakeProvider([catalog_candidate()]), source_registry=SourceRegistry(), pdf_service=CatalogPdfService(tmp_path, sha256="f" * 64), scoring=ScoringPolicy(), settings=Settings(telegram_bot_token="123456:fixture", owner_telegram_id=1, temp_dir=str(tmp_path), search_candidate_limit=1))
    with pytest.raises(NoSuitableCasesError):
        await service.prepare(1, "law_intro")
...
async def test_pdf_stage_does_not_block_asyncio_event_loop(tmp_path: Path) -> None:
    ...
    await db.initialize()
    service = CaseWorkflowService(database=db, subject_loader=SubjectLoader(), research_provider=FakeProvider([]), source_registry=SourceRegistry(), pdf_service=FakePdfService(tmp_path), scoring=ScoringPolicy(), settings=Settings(telegram_bot_token="123456:fixture", owner_telegram_id=1, temp_dir=str(tmp_path), pdf_processing_timeout_seconds=2))
...
async def test_reservation_blocks_same_case_for_another_session(tmp_path: Path) -> None:
    ...
    await db.initialize()
    assert await db.reserve_case("one", case_number="55", court_name="محكمة", pdf_sha256="d" * 64)
    assert not await db.reserve_case("two", case_number="55", court_name="محكمة", pdf_sha256="e" * 64)
    await db.release_reservation("one")
    assert await db.reserve_case("two", case_number="55", court_name="محكمة", pdf_sha256="e" * 64)
